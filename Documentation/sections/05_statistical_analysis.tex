...
To quantify interactions such as SNR $\times$ overlap ratio and model $\times$ overlap ratio,
a regression-style analysis is applied on clip-level scores. Let the design factors be:
overlap ratio (OR), unique speaker count ($K$), SNR, noise type, ASR model. The analysis focuses on:
\begin{itemize}
  \item \textbf{main effects}: OR, $K$, SNR,  model;
  \item \textbf{key interactions}: OR$\times$SNR, OR$\times$model, SNR$\times$model.
\end{itemize}
